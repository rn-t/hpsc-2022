...
int main(int argc, char** argv) {
  ...
  Body ibody[N/size], jbody[N/size], bufferbody[N/size];
  srand48(rank);
  ...
  MPI_Win win;
  MPI_Win_create(jbody, N*sizeof(Body)/size, sizeof(Body), MPI_INFO_NULL, MPI_COMM_WORLD, &win);
  
  for(int irank=0; irank<size; irank++) {
    //copy jbody to bufferbody
    for(int i=0; i<N/size; i++){
      bufferbody[i].x = jbody[i].x;
      bufferbody[i].y = jbody[i].y;
      bufferbody[i].m = jbody[i].m;
      bufferbody[i].fx = jbody[i].fx;
      bufferbody[i].fy = jbody[i].fy;
    } 
    MPI_Win_fence(0, win);
    MPI_Put(bufferbody, N/size, MPI_BODY, send_to, 0, N/size, MPI_BODY, win);
    MPI_Win_fence(0, win);
